...
2. 参考 tracking 轨迹
   ...
   - 由 `mimic_lite.py` 中的观测类切成未来多帧参考量
...
### 4.4 `mimic_lite.py` 里的 observation 是参考动作编码器

`sim2real/rl_policy/observations/motion.py` 中的 `motion_obs` 是共享的 standard motion observation 基类，`sim2real/rl_policy/observations/mimic_lite.py` 里定义具体的 mimic-lite reference observation。`motion_obs` 会：
