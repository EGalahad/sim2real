...
# scene = "./data/robots/g1/g1_29dof_rev_1_0.xml"  # Robot scene, for Sim2Sim
...
class MuJoCoMocapViewer:
    def __init__(
        ...
        joint_pos_ip: str = "10.42.0.83",
        joint_pos_port: int = 5555,
        root_pose_ip: str = "localhost",
        root_pose_port: int = 5556,
        box_pose_ip: str = "localhost",
        box_pose_port: int = 5558,
        record: bool = False,
    ):
        self.record = record
        self.freq = frequency
        ...
        # Initialize box pose subscriber (local machine)
        self.box_pose_subscriber = self.zmq_context.socket(zmq.SUB)
        self.box_pose_subscriber.connect(f"tcp://{box_pose_ip}:{box_pose_port}")
        self.box_pose_subscriber.setsockopt(zmq.SUBSCRIBE, "box".encode('utf-8'))
        self.box_pose_subscriber.setsockopt(zmq.RCVTIMEO, 100)  # 100ms timeout
        ...
        print(f"Connected to box_pose publisher at {box_pose_ip}:{box_pose_port}")
...
def main():
    ...
    parser.add_argument("--joint_pos_ip", type=str, default="10.42.0.83", help="IP address for joint position publisher")
